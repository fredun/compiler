...
tests = testGroup "Unification"
  ...
  [ testGroup "mostGeneralUnifier"
    ...
    , testCase "succeeds with a type variable on the left" $ do
        ...
        let right = Fix (Fix.Ann Set.empty (Type.Constant Type.FunctionConstant))
    ...
    , testCase "succeeds with a type variable on the right" $ do
        let left = Fix (Fix.Ann Set.empty (Type.Constant Type.FunctionConstant))
        let right = Fix (Fix.Ann (Set.singleton (Type.Identifier "a")) (Type.Variable (Type.Identifier "a")))
